...
本项目优先服务不懂技术、描述不一定清晰、遇到断网容易着急的普通用户。处理需求时先按产品经理视角理解用户想完成的结果，再决定技术实现。

- 用户说的是场景、感受和目标，不一定是准确的技术方案
- 不要求用户掌握 Python、命令行、认证协议、配置路径等专业概念
- 面向普通用户的入口优先使用 GUI 和清晰按钮，避免把命令行作为唯一路径
- 报错信息要说明“现在发生了什么、用户该点哪里或做什么”，不要只暴露异常名
- 需要用户选择时，给少量明确选项；默认选项要适合大多数普通用户
- 用户脾气急时，优先减少步骤、保留已有配置、避免要求重复输入账号密码
- 技术重构不能牺牲双击即用、开机守护、多账号切换这些核心体验
- 文档先写用户能看懂的操作结果，再补高级用户的技术细节
