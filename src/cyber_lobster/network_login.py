...
def inject_cookies(session: requests.Session,
    ...
    注意: 明文密码绝不能放入 Cookie，密码仅通过 form body RSA 加密传输。
    """
    jar = {
...
if __name__ == "__main__":
    ...
    uid = input("学号: ").strip()
    pwd = _getpass.getpass("密码（输入时不显示）: ")
    svc = input("运营商 [DX]: ").strip() or "DX"

    creds = PortalCredentials(user_id=uid, password=pwd, service=svc)
